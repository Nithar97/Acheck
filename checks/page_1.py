...
def error_check1():
    combined_data, filtered_data = process_and_display_data()
    if filtered_data is not None:
        ...
        # df_profile = combined_data[columns_to_display]

        # pr = ProfileReport(df_profile, explorative=True)
        # st.header('**Pandas Profiling Report**')
        # st_profile_report(pr)

        # pr = combined_data.profile_report()
        # st_profile_report(pr)
        ...
        if st.session_state.has_clicked:
            ...
            st.subheader('Upload Exceptions Dataset')
